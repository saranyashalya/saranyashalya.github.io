...
// "system". Default is "light" (clean, professional aesthetic for first-time visitors).
//
// Migration: an earlier build defaulted to "dark" and persisted it to localStorage on
// first load. Bump THEME_DEFAULT_VERSION whenever the default changes — visitors whose
// stored version is older get reset once so the new default applies.
const THEME_DEFAULT_VERSION = "2026-05-22-light";
let determineThemeSetting = () => {
  let storedVersion = localStorage.getItem("theme-default-version");
  if (storedVersion !== THEME_DEFAULT_VERSION) {
